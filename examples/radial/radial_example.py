...
radial_sobol_result = radial_ee.analyze(
    problem, radial_sobol_param, radial_sobol_Y, sample_sets
)

# Radial MC
radial_mc_param = radial_mc.sample(problem, mc_sample_sets)
radial_mc_Y = Ishigami.evaluate(radial_mc_param)
radial_mc_result = radial_ee.analyze(
    problem, radial_mc_param, radial_mc_Y, mc_sample_sets
)
...
Si = sobol.analyze(
    problem, sobol_Y, calc_second_order=True, conf_level=0.95, print_to_console=False
)
