...
from ..sample.morris import sample, optimize_trajectories
...
def test_find_optimum_trajectories():
    ...
    output = optimize_trajectories(problem, input_sample, N, k_choices)
...
def test_group_sample_fails_with_wrong_G_matrix():
    ...
    num_levels = 4
    grid_jump = 2
    problem = {'groups': (list[1,2,3,4], None)}
    sample(problem, N, num_levels, grid_jump)
