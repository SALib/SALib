...
def analyze(
    ...
    * `radial_mc`
    ...
    assert (
        X.shape[0] / sample_sets
    ) == nth, "Number of parameter set groups must match number of parameters + 1"

    assert (
        Y.shape[0] / sample_sets
    ) == nth, "Number of result set groups must match number of parameters + 1"
    ...
    Si = ResultDict((k, [None] * num_vars) for k in ["names", "mu", "mu_star", "sigma"])

    Si["mu"] = np.average(ee, axis=0)
    Si["mu_star"] = np.average(np.abs(ee), axis=0)
    Si["mu_star_conf"] = compute_radial_ee_confidence(
        ee, sample_sets, num_resamples, conf_level
    )
...
def compute_radial_ee_confidence(
    ee: np.array, N: int, num_resamples: int, conf_level: float = 0.95
) -> np.array:
