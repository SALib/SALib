...
def test_scale_samples():
    ...
    desired = np.array([np.arange(10, 21, 1), np.arange(-10, 12, 2)], dtype=float).T
...
def test_unscale_samples():
    ...
    """
    params = np.array([np.arange(10, 21, 1), np.arange(-10, 12, 2)], dtype=float).T
    bounds = [[10, 20], [-10, 10]]
...
def test_compute_groups_from_parameter_file():
    ...
    """
    actual_matrix, actual_unique_names = compute_groups_matrix(
        ["Group 1", "Group 2", "Group 2"]
    )

    assert_equal(actual_matrix, np.array([[1, 0], [0, 1], [0, 1]], dtype=int))
