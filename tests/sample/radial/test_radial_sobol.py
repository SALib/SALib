...
def test_radial_jansen_G4():
    ...
    delta = np.random.RandomState(101).rand(20)
    r_X = r_sample(problem, num_sample_sets, seed=101)
    r_Y = Sobol_G.evaluate(r_X, a, alpha=alpha, delta=delta)

    g4_result = j_analyze(problem, r_Y, num_sample_sets, num_resamples=1000, seed=101)
    ...
    assert num_analytic == 4, "Expected 4 analytic values, but only found {}".format(
        num_analytic
    )

    assert np.array_equal(
        np.where(analytic_vals > 0.05), np.where(ST_res > 0.05)
...
def test_radial_jansen_G10():
    ...
    g10_result = j_analyze(problem, r_Y, num_sample_sets, num_resamples=1000, seed=101)
    ST_res = g10_result["ST"].round(4)

    assert_allclose(analytic_vals, ST_res, atol=0.04, rtol=0.04)

    num_analytic = len(np.where(analytic_vals > 0.05)[0])
    assert num_analytic == 10, "Expected 10 analytic values, but only found {}".format(
        num_analytic
    )
