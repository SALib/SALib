...
def test_ff():
    cmd = "python {cli} sample ff -p {fn} -r test.txt -n 100".format(
        cli=salib_cli,
        fn=ishigami_fp)
...
def test_fast():
    cmd = "python {cli} sample fast_sampler -p {fn} -r test.txt -n 100".format(
        cli=salib_cli,
        fn=ishigami_fp)
    result = subprocess.check_output(cmd)
    assert len(result) == 0, "Error occurred!"
...
def test_finite_diff():
    cmd = "python {cli} sample finite_diff -p {fn} -r test.txt -n 100".format(
        cli=salib_cli,
        fn=ishigami_fp)
    result = subprocess.check_output(cmd)
    assert len(result) == 0, "Error occurred!"
...
def test_latin():
    cmd = "python {cli} sample latin -p {fn} -r test.txt -n 100".format(
        cli=salib_cli,
        fn=ishigami_fp)
    result = subprocess.check_output(cmd)
    assert len(result) == 0, "Error occurred!"
